...
public:
    ...
    TextureData* getData(const std::string& filename)
    {
        ...
        // Cache the texture
        TextureData data{filename, surface, texture, surface->w, surface->h, static_cast<double>(surface->w) / surface->h};
        textureMap.insert({filename, data});
...
void drawScaled(const std::string &filename, double centerX, double centerY, double width, double height, double rotation, double scale)
{
    SDL_Texture *texture = get(filename);
    if (texture)
    {
        drawScaled(texture, centerX, centerY, width, height, rotation, scale);
    }
}
...
void drawPreserveAspect(const std::string &filename, double centerX, double centerY, double width, double height, double rotation, double scale) {
    auto *data = getData(filename);
    if (data) {
        if (data->aspect < 1.0) {
            drawScaled(filename, centerX, centerY, width, width / data->aspect, rotation, scale);
        } else {
            drawScaled(filename, centerX, centerY, height * data->aspect, height, rotation, scale);
        }
    }
}
